...
test_that("array patterns works as expected", {
	...
	test_raster("gradient.png", function() {
	...
	test_raster("gradient_horizontal.png", function() {
		grid.pattern_gradient(
			x,
			y,
			fill = "blue",
	...
	test_raster("placeholder.png", function() grid.pattern_placeholder(x, y, type = "bear"))

	test_raster("plasma_zero.png", function() {
		grid.pattern_plasma(x = c(0.5, 0.5, 0.5, 0.5), y = c(0, 1, 1, 0), fill = "green")
	})

	test_raster("plasma.png", function() {
		magick::magick_set_seed(42) # introduced in v2.7.4
		grid.pattern_plasma(x, y, fill = "green")
	})

	playing_card_symbols <- c("\u2660", "\u2665", "\u2666", "\u2663")
	test_raster("text.png", function() {
		grid.pattern_text(
			x,
			y,
	...
	clipper <- grid::pathGrob(
		x = c(x_hex_outer, x_hex_inner),
		y = c(y_hex_outer, y_hex_inner),
		id = rep(1:2, each = 7),
		rule = "evenodd"
	)

	clipped <- clippingPathGrob(
		clippee,
		clipper,
	...
	test_raster("ambient.png", function() {
		set.seed(42)
		grid.pattern_ambient(x, y, fill = "green", fill2 = "blue")
	})
	test_raster("ambient_worley.png", function() {
		set.seed(42)
