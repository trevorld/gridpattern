...
```{r circle, fig.alt = "The 'circle' pattern"}
grid.pattern_circle(x_hex, y_hex, density = 0.5, grid = "hex_circle",
                    fill = c("blue", "yellow", "red"))
```
```{r regular_star, fig.alt = "The 'regular_polygon' pattern"}
...
```
```{r regular_hex, fig.alt = "The 'regular_polygon' pattern used to create a hexagonal tiling"}
...
```
```{r truncated_hexagonal, fig.alt = "The 'polygon_tiling' pattern used to create a truncated hexagonal tiling"}
gp <- gpar(fill = c("yellow", "blue", "red"))
grid.pattern_polygon_tiling(x_hex, y_hex, type = "truncated_hexagonal",
                            spacing = 0.15, gp = gp)
```
```{r rhombitrihexagonal, fig.alt = "Using several patterns to create a rhombitrihexgonal tiling variation"}
...
grid.pattern_regular_polygon(shape = "star12", grid = "hex_circle", rot = 15,
                             ...
                             scale = scale)
```
```{r rose, fig.alt = "The 'rose' pattern"}
...
```
```{r text, fig.alt = "The 'text' pattern with playing card suits"}
...
```
```{r wave, fig.alt = "The 'wave' pattern"}
...
```
```{r weave, fig.alt = "Using the 'weave' pattern to make a 'satin' weave"}
...
```{r ggpattern, fig.alt = "A bar chart using 'ggpattern' with builtin patterned fills"}
...
```{r hex_ggpattern, fig.alt = "A bar chart using 'ggpattern' with custom patterned fills"}
...
```{r ggplot2, fig.width=9, fig.height=9, fig.alt = "A bar chart using 'ggplot2' with 'patternFill()' 'fill' values"}
library("ggplot2", warn.conflicts = FALSE)
library("gridpattern")
...
```{r piecepackr, fig.width=9, fig.height=9, fig.alt = "A board game diagram using 'piecepackr' with custom patterned game pieces"}
library("grid")
library("gridpattern")
library("piecepackr")
